...
def _helper(netG, x_tilde, eps, sigma):
    ...
    with torch.no_grad():
        G_eps = netG(eps)
    b_size = eps.size(0)
    log_prob_eps = (eps**2).view(b_size, -1).sum(1).view(-1, 1)
    log_prob_x = (x_tilde - G_eps)**2 / sigma**2
    log_prob_x = log_prob_x.view(b_size, -1)
...
def get_samples(netG, x_tilde, eps_init, sigma, burn_in, num_samples_posterior, leapfrog_steps, stepsize, flag_adapt, hmc_learning_rate, hmc_opt_accept):
    device = eps_init.device
    b_size, eps_dim = eps_init.size(0), eps_init.size(1)
    n_steps = burn_in + num_samples_posterior
    acceptHist = torch.zeros(b_size, n_steps).to(device)
    logjointHist = torch.zeros(b_size, n_steps).to(device)
    samples = torch.zeros(b_size*num_samples_posterior, eps_dim).to(device)
    ...
    for i in range(n_steps):
        ...
        current_p = p
        logjoint_vec, logjoint, grad_logjoint = _helper(netG, x_tilde, current_eps, sigma)
        current_U = -logjoint_vec.view(-1, 1)
        ...
        for j in range(leapfrog_steps):
            ...
            if j < leapfrog_steps - 1:
                logjoint_vec, logjoint, grad_logjoint = _helper(netG, x_tilde, current_eps, sigma)
                proposed_U = -logjoint_vec
                grad_U = -grad_logjoint
                p = p - stepsize * grad_U
        logjoint_vec, logjoint, grad_logjoint = _helper(netG, x_tilde, eps, sigma)
        proposed_U = -logjoint_vec.view(-1, 1)
        grad_U = -grad_logjoint
        p = p - stepsize * grad_U / 2.0
        p = -p
        current_K = 0.5 * (current_p**2).sum(dim=1)
        current_K = current_K.view(-1, 1)
        proposed_K = 0.5 * (p**2).sum(dim=1)
        proposed_K = proposed_K.view(-1, 1)
